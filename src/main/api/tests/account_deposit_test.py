...
class TestAccountDeposit:
    ...
    @pytest.mark.parametrize(
        "amount", [500.5, 999.9, 9000.1, -500.5]
    )
    def test_account_deposit_invalid(self, db_session: Session, api_manager: ApiManager, create_user_request: CreateUserRequest, create_user_account: CreateAccountResponse, amount: float):
        response = api_manager.user_steps.account_deposit_invalid(create_user_account, create_user_request, amount)
        account_from_db = AccountCrudDb.get_account_by_id(db_session, response.id)

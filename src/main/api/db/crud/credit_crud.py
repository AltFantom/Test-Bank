...
class CreditCrudDb:
    ...
    @staticmethod
    def get_credits_by_account_id(db: Session, account_id: int) -> Credit | None:
        return db.query(Credit).join(Account, Credit.account_id == Account.id).filter(Account.id == account_id).first()